...
from app.web.log_parser import parse_log_file, get_all_logs_info, get_latest_log_info
from app.web.log_handler import get_logs, capture_session_logs
...
async def websocket_endpoint(websocket: WebSocket, session_id: str):
    try:
        ...
        while session["status"] == "processing":
            ...
            current_thinking_step_count = len(current_thinking_steps)
            ...
            current_tracker_logs = ThinkingTracker.get_logs(session_id)
            current_tracker_log_count = len(current_tracker_logs)
            if current_tracker_log_count > last_tracker_log_count:
                await websocket.send_text(
                    json.dumps(
                        {
                            "status": session["status"],
                            "logs": current_tracker_logs[last_tracker_log_count:],
                        }
                    )
                )
        ...
        if (
            ...
            await websocket.send_text(json.dumps(final_notification))
            # 正常关闭WebSocket连接
            print(f"WebSocket 任务完成，关闭连接: {session_id}")
            await websocket.close()
...
async def process_prompt(session_id: str, prompt: str):
    ...
    try:
        ...
        with capture_session_logs(session_id) as log:
            ...
            ThinkingTracker.add_thinking_step(
                session_id,
                f"分析用户请求: {prompt[:50]}{'...' if len(prompt) > 50 else ''}",
            )
            ...
            # 创建更有意义的结论，包含实际的结果内容
            conclusion_message = f"任务处理完成！已在工作区 {workspace_dir.name} 中生成结果。"

            # 添加带有详细结果的结论
            ThinkingTracker.add_conclusion(
                session_id,
                conclusion_message,
                details=result,  # 将实际结果作为详细信息传递
            )

            # 还要添加一个特殊的"answer"类型步骤，明确指出最终答案
            ThinkingTracker.add_thinking_step(
                session_id,
                "根据以上分析，最终回答如下:",
                step_type="answer",
